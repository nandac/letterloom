/// test-letterhead-sender-position-left-top
///
/// Synopsis:
/// Rendering test that validates the sender-position: left, sender-valign: top
/// layout, where the sender address is aligned to the top edge of the letterhead
/// image.
///
/// Purpose:
/// Ensures that sender-valign: top correctly places the sender block flush with
/// the top of the image when sender-position is left, rather than the default
...
/// - letterhead.sender-valign: top
/// - letterhead.image-inset: (top: 10mm, rest: 5mm)
///
/// Expected Behavior:
/// - Page 1: sender name and address appear on the left, aligned to the top edge
///   of the image; letterhead image occupies the right 65mm; letter content
///   (date, recipient, body) flows below both
/// - Page 2+: no letterhead; normal margins apply throughout
///
/// Validation:
/// Output is compared to reference images to detect regressions in top-aligned
/// sender placement alongside a left-position letterhead.
#import "/src/lib.typ": *
