/// test-letterhead-sender-position-left
///
/// Synopsis:
/// Rendering test that validates the sender-position: left layout, where the
/// letterhead image sits flush against the right page edge and the sender address
/// fills the remaining column to the left, vertically centred relative to the
/// image (default sender-valign: horizon).
///
/// Purpose:
/// Ensures that construct-letterhead correctly splits the header row when
/// sender-position is left, placing the image in a right-bounded column of the
/// specified width and the sender in the remaining left-side space, centred
...
///
/// Expected Behavior:
/// - Page 1: sender name and address appear on the left, vertically centred
///   relative to the image; letterhead image occupies the right 65mm; letter
